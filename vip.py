        ...
        if lenin < 6:
           ...
           if(len(idx[0]) == 0):
               ...
               if (lenin==5):
                   ...
                   print('The vIP value of the single stranded DNA "',x,'" is',round(PP, 3),'eV')
        if (lenin>=6):
           CECCA=lenin-3;
        ...
        if lenin < 6:
           ...
           if(len(idx[0]) == 0):
               ...
               if (lenin==5):
                   ...
                   seconvip=np.where(Seq == second)
                   DUE=vIP[seconvip[0][0]]
                   PP1 = DL+(0.38685 * (float(UNO)-DLMENO1)+(1-0.38685)*(float(DUE)-DLMENO1))
                   primo=y[:-1]
                   primovip=np.where(Seq == primo)
                   UNO=vIP[primovip[0][0]]
                   second=y[1:]
                   seconvip=np.where(Seq == second)
                   DUE=vIP[seconvip[0][0]]
                   PP2 = DL+(0.38685 * (float(UNO)-DLMENO1)+(1-0.38685)*(float(DUE)-DLMENO1))
                   print('The vIP value of the double stranded DNA "',x,'" is',round((PP1+PP2)/2,3),'eV')
        if (lenin>=6):
           CECCA=lenin-3;
           POLLO = ["" for l in range(CECCA)]
           RISULTATO=np.zeros(CECCA,float)
